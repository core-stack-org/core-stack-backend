...
def extract_ndmis(feature):
    ...
    ndmi_properties = [
        '100m_NDMI', '150m_NDMI','200m_NDMI', '250m_NDMI', '300m_NDMI', '350m_NDMI',
        '400m_NDMI', '450m_NDMI', '500m_NDMI', '550m_NDMI', '600m_NDMI',
        '650m_NDMI', '700m_NDMI', '750m_NDMI', '800m_NDMI', '850m_NDMI',
        '900m_NDMI', '950m_NDMI', '1000m_NDMI', '1050m_NDMI', '1100m_NDMI',
        '1150m_NDMI', '1200m_NDMI', '1250m_NDMI', '1300m_NDMI', '1350m_NDMI',
        '1400m_NDMI', '1450m_NDMI', '1500m_NDMI'
    ]
...
def classify_checkdams(ndmi_list, poly_degree=2, threshold=0):
    ...
    zoi_results = {}
    for idx, ndmi_values in enumerate(ndmi_list):  # Loop through each list of NDMI values (each checkdam)
        # Get smoothed data, intervals, and slopes
        smoothened_y_full, intervals, slopes = calculate_slopes_with_smoothing(ndmi_values, poly_degree)
        if (idx==2):
          print("slopes: ")
          slope_arr = []
          for slope in slopes:
...
def classify_checkdams_updated(ndmi_list, poly_degree=2, threshold=0):
    ...
    for idx, ndmi_values in enumerate(ndmi_list):
        smooth_y, intervals, slopes = calculate_slopes_with_smoothing(ndmi_values, poly_degree)
