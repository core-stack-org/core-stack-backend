...
def get_mws_features(state, district, block):
    asset_id = get_gee_asset_path(state, district, block) + "filtered_mws_" + valid_gee_text(district.lower()) + "_" + valid_gee_text(block.lower()) + "_uid"
...
def tree_health_ccd_vector(self, state, district, block, start_year, end_year):
    ...
    for uid, data in uid_data.items():
        matching_feature = next(
            (f for f in mws_info['features'] if f['properties']['uid'] == uid),
            None
        )
    ...
    save_layer_info_to_db(
        ...
        district, 
        block, 
        layer_name = f"{district.title()}_{block.title()}_tree_health_ccd_vector",
        asset_id="no asset id",
        workspace_name = "Ccd Vector"
        )
