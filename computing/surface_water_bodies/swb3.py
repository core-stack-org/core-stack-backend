...
def add_village_name_flag(
    ...
    if village_fc is None:
        return swb_fc.map(
            lambda f: ee.Feature(f).set(
                {
                    # Table export does not support List<String>; store JSON string.
    ...
    def classify(feature):
        ...
        intersecting_villages = ee.Algorithms.If(
            has_matches,
            ee.String.encodeJSON(names),  # JSON array string
            "[]",
        )
        intersecting_villages_count = ee.Algorithms.If(has_matches, names.size(), 0)
        covering_village_names = ee.Algorithms.If(has_matches, ee.String(names.join(", ")), None)
...
def waterbody_catchment_streamorder_properties(
    ...
    # Swapped naming: enriched final layer now exports as SWB3.
    ...
    supporting_asset_suffix = supporting_asset_suffix or asset_suffix
