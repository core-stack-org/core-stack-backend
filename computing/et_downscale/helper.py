...
def crop_year_start_month(cfg: dict | None = None) -> int:
    if cfg is None:
        return DEFAULT_CROP_YEAR_START_MONTH
    value = cfg.get("crop_year_start_month", DEFAULT_CROP_YEAR_START_MONTH)
    value = int(value)
    if value != DEFAULT_CROP_YEAR_START_MONTH:
        raise ValueError(
            "Core ET products use a fixed July-June crop year; "
            "crop_year_start_month must be 7."
        )
    return value
...
def crop_month_abbrs(start_month: int = DEFAULT_CROP_YEAR_START_MONTH) -> list[str]:
    return [
        calendar.month_abbr[((start_month - 1 + idx) % 12) + 1]
        for idx in range(12)
    ]


def crop_month_order(start_month: int = DEFAULT_CROP_YEAR_START_MONTH) -> str:
    return ",".join(crop_month_abbrs(start_month))
...
def crop_year_end_month(start_month: int = DEFAULT_CROP_YEAR_START_MONTH) -> int:
    return ((int(start_month) + 10) % 12) + 1


def crop_year_end_year(
    year: int,
    start_month: int = DEFAULT_CROP_YEAR_START_MONTH,
) -> int:
    return int(year) if int(start_month) == 1 else int(year) + 1


def crop_year_label(year: int, start_month: int = DEFAULT_CROP_YEAR_START_MONTH) -> str:
    year = int(year)
    end_year = crop_year_end_year(year, start_month)
    if end_year == year:
        return str(year)
    return f"{year}_{end_year}"


def crop_year_span_label(
    year: int,
    start_month: int = DEFAULT_CROP_YEAR_START_MONTH,
) -> str:
    start_month = int(start_month)
    end_month = crop_year_end_month(start_month)
    end_year = crop_year_end_year(year, start_month)
    return (
        f"{calendar.month_abbr[start_month]} {int(year)}-"
        f"{calendar.month_abbr[end_month]} {end_year}"
    )
...
def asset_suffix(cfg: dict) -> str:
    return _asset_token(cfg.get("asset_suffix") or cfg.get("tehsil_name") or "tehsil")


def _build_asset_id(cfg: dict, label: str, year: int | None = None) -> str:
    root = str(cfg.get("asset_root", "")).rstrip("/")
    if not root:
        raise ValueError("asset_root is required")
    year = int(cfg["year"] if year is None else year)
    return f"{root}/{label}_{asset_suffix(cfg)}_{year}"


def product_asset_id(cfg: dict, label: str, year: int | None = None) -> str:
    return _build_asset_id(cfg, label, year=year)
...
def ee_annual_total_band(
    ...
    for crop_idx in range(12):
        month_band = monthly_stack.select(f"{prefix}_{crop_idx + 1:02d}")
        actual_month = ((start_month - 1 + crop_idx) % 12) + 1
    ...
    # A crop-year total is meaningful only when all 12 monthly values exist.
...
def common_metadata(cfg: dict, application: str) -> dict:
    start_month = crop_year_start_month(cfg)
    metadata = {
        "application": application,
        "year": str(cfg["year"]),
        "crop_year_label": crop_year_label(cfg["year"], start_month),
        "crop_year_span": crop_year_span_label(cfg["year"], start_month),
        "crop_year_start_month": str(start_month),
        "crop_year_end_month": str(crop_year_end_month(start_month)),
        "crop_year_month_order": crop_month_order(start_month),
        "asset_suffix": asset_suffix(cfg),
    }
    if cfg.get("tehsil_name"):
        metadata["tehsil"] = cfg["tehsil_name"]
    if cfg.get("tehsil_asset"):
        metadata["tehsil_asset"] = cfg["tehsil_asset"]
    return metadata
    ...
    while pending:
        ...
        for asset_id, spec in pending.items():
            if not spec.get("task"):
                ...
                final_statuses[asset_id] = {"state": "SKIPPED_EXISTING_ASSET"}
                continue
        ...
        if pending:
            print(
                f"  Still running: {len(pending)} task(s). "
...
def export_product_asset(
    ...
    already_exists = _prepare_asset_target(
    ...
    if not already_exists:
        task = _start_asset_export(
            ...
            asset_id,
            description=(
                f"export_{label}_{asset_suffix(cfg)}_{int(cfg['year'])}"
            ),
        )
...
def fill_monthly_collection(
    ...
    """
    Fill masked monthly pixels from neighbouring crop-year months.

    August-May use the requested +/-45-day window. July (month 1) can use
    August only, and June (month 12) can use May only, so interpolation never
    crosses a crop-year boundary. Pixels that still have no value stay masked
    and are written as NODATA only by ``finalize_export_image``.
    """
...
def interpolate_monthly_stack(
    ...
    return monthly_collection_to_stack(
        filled_monthly,
        value_band,
        f"{prefix}_",
        region,
