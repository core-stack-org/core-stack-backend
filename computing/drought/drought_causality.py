...
def data_with_column(year, gdf, fin_df):
    ...
    fin_df[f'frequency_of_no_drought_{year}'] = round(gdf[f'freq_of_drought_{year}_at_threshold_0'],2)
    fin_df[f'frequency_of_mild_drought_{year}'] = round(gdf[f'freq_of_drought_{year}_at_threshold_1'],2)
    fin_df[f'frequency_of_moderate_drought_{year}'] = round(gdf[f'freq_of_drought_{year}_at_threshold_2'],2)
    fin_df[f'frequency_of_severe_drought_{year}'] = round(gdf[f'freq_of_drought_{year}_at_threshold_3'],2)
    fin_df[f'intensity_of_no_drought_{year}'] = round(gdf[f'intensity_of_drought_{year}_at_threshold_0'],2)
    fin_df[f'intensity_of_mild_drought_{year}'] = round(gdf[f'intensity_of_drought_{year}_at_threshold_1'],2)
    fin_df[f'intensity_of_moderate_drought_{year}'] = round(gdf[f'intensity_of_drought_{year}_at_threshold_2'],2)
    fin_df[f'intensity_of_severe_drought_{year}'] = round(gdf[f'intensity_of_drought_{year}_at_threshold_3'],2)
    ...
    fin_df[f'mild_drought_vci_score_{year}'] = (fin_df[f'mild_drought_vci_score_{year}'] / 6).round(2)
    fin_df[f'mild_drought_mai_score_{year}'] = (fin_df[f'mild_drought_mai_score_{year}'] / 6).round(2)
    fin_df[f'mild_drought_cropping_area_sown_score_{year}'] = (fin_df[f'mild_drought_cropping_area_sown_score_{year}'] / 6).round(2)
    return fin_df
    ...
    for year in range(start_year, end_year + 1):
        ...
        for feature in features:
            ...
            if not matching_row.empty:
                ...
                combined_uid_data[uid][f'mild_{year}'] = convert_to_dict(str(matching_row[f'mild_drought_causality_{year}'].iloc[0]))

        # Export individual year data if needed
        features = []
        for _, row in data.iterrows():
            feature = next(f for f in mws_features if f['properties']['uid'] == row['uid'])
