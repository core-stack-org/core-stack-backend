...
from utilities.constants import GREEN_CREDIT_PAN_INDIA_DATASET
...
TASK_NAME = "generate_green_credit_data"


@app.task(bind=True)
def generate_green_credit_data(self, state, district, block, gee_account_id):
    """
    It will generate green credit layer for given location at tehsil level
    """
    ...
    try:
        ...
        pan_india_asset_id = GREEN_CREDIT_PAN_INDIA_DATASET

        description = (
            f"{valid_gee_text(district.lower())}_{valid_gee_text(block.lower())}_green_credit"
        )
        ...
        if not is_gee_asset_exists(roi_asset_id):
            raise RuntimeError(
                f"MWS filtered layer required before green credit generation: "
                f"{roi_asset_id} not found. Run MWS layer generation first."
            )
        if not is_gee_asset_exists(pan_india_asset_id):
            raise RuntimeError(
                f"Green credit pan-India dataset not found: {pan_india_asset_id}"
            )
        ...
        if feature_count == 0:
            log_task_step(
                TASK_NAME,
                "no_green_credit_features",
                message="No green credit features in this block; skipping export",
                **ctx,
            )
        ...
        if not is_gee_asset_exists(asset_id):
            ...
            if not task_id:
                raise RuntimeError(
                    f"Failed to start GEE export for green credit layer at {asset_id}"
                )
        ...
        if not is_gee_asset_exists(asset_id):
            raise RuntimeError(
                f"Green credit GEE asset was not created at {asset_id}. "
        ...
        log_task_step(
            TASK_NAME, "sync_fc_to_geoserver", workspace="green_credit", **ctx
        )
        ...
        if not layer_at_geoserver:
            raise RuntimeError(
                f"Green credit layer GeoServer sync failed for workspace=green_credit "
