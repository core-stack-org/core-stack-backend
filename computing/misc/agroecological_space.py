...
TASK_NAME = "generate_agroecological_data"


@app.task(bind=True)
def generate_agroecological_data(self, state, district, block, gee_account_id):
    """
    It will generate agroecological layer for given location at tehsil level
    """
    ...
    try:
        ...
        description = (
            f"{valid_gee_text(district.lower())}_{valid_gee_text(block.lower())}_agroecological"
        )
        ...
        if not is_gee_asset_exists(roi_asset_id):
            raise RuntimeError(
                f"MWS filtered layer required before agroecological generation: "
                f"{roi_asset_id} not found. Run MWS layer generation first."
            )
        if not is_gee_asset_exists(pan_india_asset_id):
            raise RuntimeError(
                f"Agroecological pan-India dataset not found: {pan_india_asset_id}"
            )
        ...
        if feature_count == 0:
            log_task_step(
                TASK_NAME,
                "no_agroecological_features",
                message="No agroecological features in this block; skipping export",
                **ctx,
            )
        ...
        if not is_gee_asset_exists(asset_id):
            ...
            if not task_id:
                raise RuntimeError(
                    f"Failed to start GEE export for agroecological layer at {asset_id}"
                )
        ...
        if not is_gee_asset_exists(asset_id):
            raise RuntimeError(
                f"Agroecological GEE asset was not created at {asset_id}. "
        ...
        log_task_step(
            TASK_NAME, "sync_fc_to_geoserver", workspace="agroecological", **ctx
        )
        ...
        if not layer_at_geoserver:
            raise RuntimeError(
                f"Agroecological layer GeoServer sync failed for workspace=agroecological "
