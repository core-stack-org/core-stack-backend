...
@app.task(bind=True)
def clip_drainage_lines(self, state, district, block, user):
    try:
        # Load the input file
        ...
        if not result_gdf.empty:
            ...
            push_shape_to_geoserver(output_shapefile_path, workspace="drainage")
             # Generated Dataset data to db 
            layer_name = f"{district}_{block}"

            try:
                return create_dataset_for_generated_layer(state, district, block, layer_name, user, gee_path=None, layer_type='vector', workspace='drainage', algorithm=None, version=None, style_name=None, misc=None)
            except Exception as e:
                print(f"Exception while creating entry for drainage in dataset table: {str(e)}")
        else:
