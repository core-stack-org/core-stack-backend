...
def _output_dir(state: str, district: str, block: str) -> Path:
    return _repo_path(LOCAL_ANTYODAYA_2020_OUTPUT) / _slug(state) / _slug(district) / _slug(block)
...
def _location_rows() -> tuple[tuple[str, str, str], ...]:
    source_path = _repo_path(PAN_INDIA_ANTYODAYA_2020)
...
def generate_antyodaya_layer_task(
    ...
    return {
        ...
        "rows": int(len(gdf)),
        "source": _repo_path(PAN_INDIA_ANTYODAYA_2020).as_posix(),
