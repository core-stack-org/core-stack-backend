...
from utilities.constants import FACTORY_PAN_INDIA_DATASET
...
TASK_NAME = "generate_factory_csr_data"


@app.task(bind=True)
def generate_factory_csr_data(self, state, district, block, gee_account_id):
    """
    It will generate factory csr data if available for given location at tehsil level
    """
    ...
    try:
        ...
        pan_india_asset_id = FACTORY_PAN_INDIA_DATASET

        description = (
            f"{valid_gee_text(district.lower())}_{valid_gee_text(block.lower())}_factory_csr"
        )
        ...
        if not is_gee_asset_exists(roi_asset_id):
            raise RuntimeError(
                f"MWS filtered layer required before factory CSR generation: "
        ...
        if feature_count == 0:
            log_task_step(
                TASK_NAME,
                "no_factory_csr_features",
                message="No factory CSR features in this block; skipping export",
                **ctx,
            )
        ...
        if not is_gee_asset_exists(asset_id):
            ...
            if not task_id:
                raise RuntimeError(
                    f"Failed to start GEE export for factory CSR layer at {asset_id}"
                )
        ...
        if not is_gee_asset_exists(asset_id):
            raise RuntimeError(
                f"Factory CSR GEE asset was not created at {asset_id}. "
        ...
        if not layer_at_geoserver:
            raise RuntimeError(
                f"Factory CSR layer GeoServer sync failed for workspace=factory_csr "
