...
def layer_status(request):
    ...
    if response_states.status_code == 200:
        ...
        states = [
            ...
            if state.get("active_status")
        ]
        states.sort(key=lambda x: x.get("state_name", "").lower())

    # Fetch districts for the selected state
    if state_census_code:
        response_districts = requests.get(
            f"https://geoserver.core-stack.org/api/v1/get_districts/{state_census_code}/"
        )
        if response_districts.status_code == 200:
            ...
            districts = [
                ...
                if district.get("active_status")
            ]
            districts.sort(key=lambda x: x.get("district_name", "").lower())

    # Fetch blocks for the selected district
    if district_id:
        response_blocks = requests.get(
            f"https://geoserver.core-stack.org/api/v1/get_blocks/{district_id}/"
        )
        if response_blocks.status_code == 200:
            ...
            blocks = [
                ...
                if block.get("active_status")
            ]
            blocks.sort(key=lambda x: x.get("block_name", "").lower())

    # Get district and blocks names
    district_name = next(
        (d["district_name"] for d in districts if str(d["id"]) == str(district_id)),
        None,
    )
    block_name = next(
        (
            b["block_name"]
            for b in blocks
            if str(b["block_census_code"]) == str(block_id)
        ),
        None,
    )
            ...
            if layer_type == "vector":
                ...
                for raster_workspace in raster_workspaces:
                    ...
                    if response.status_code != 200:
                        print(
                            f"Failed to retrieve capabilities from {raster_workspace}."
                        )
                        continue
