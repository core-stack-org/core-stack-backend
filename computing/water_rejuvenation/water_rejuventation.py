...
def get_lulc_class(ref_lat, ref_long):
    ...
    point_value = final_lulc_img.reduceRegion(
        reducer=ee.Reducer.first(), geometry=reference_point, scale=10, maxPixels=1e9
    )
...
def find_closest_water_pixel(lon, lat, lulc_class):
    ...
    if lulc_class is None:
        print(
            "Reference point has no LULC value. This could be outside the image bounds."
        )
        lulc_class = -1
    print(
        f"Reference point is not a water pixel (class {lulc_class}). Finding closest water pixel..."
    )
    ...
    water_pixel_count = (
        water_pixels.reduceRegion(
            reducer=ee.Reducer.count(), geometry=search_region, scale=10, maxPixels=1e9
        )
        .getInfo()
    ...
    min_distance_result = water_distance.reduceRegion(
        reducer=ee.Reducer.min(), geometry=search_region, scale=10, maxPixels=1e9
    )
    ...
    epsilon = 0.1
    min_dist_pixels = (
        water_distance.lte(ee.Image.constant(min_dist_value + epsilon))
        .And(water_distance.gte(ee.Image.constant(min_dist_value - epsilon)))
        .And(water_mask)
    )
    ...
    if min_dist_count > 1:
        print(
            f"Multiple pixels found at the same minimum distance. Taking the first one."
        )
...
def find_watersheds_for_point_with_buffer(latitude, longitude, buffer_distance=1500):
    ...
    intersecting_watersheds = WaterSheds.filter(
        ee.Filter.intersects(".geo", buffered_point)
    )
...
def compute_metrics(feature, elevation, ndmi_image, cropping_mask, scale=30):
    ...
    center_elevation = elevation.reduceRegion(
        reducer=ee.Reducer.mean(), geometry=point_geom, scale=scale, maxPixels=1e9
    ).get("elevation")
    ...
    for radius in buffer_radii:
        ...
        ndmi_mean = ndmi_masked_downstream.reduceRegion(
            reducer=ee.Reducer.mean(), geometry=buffer_area, scale=scale, maxPixels=1e9
        ).get("NDMI")

        pixel_count = (
            ndmi_masked_downstream.updateMask(ndmi_masked_downstream.neq(0))
            .reduceRegion(
        ...
        # Handle null values
        ndmi_mean = ee.Algorithms.If(
            ee.Algorithms.IsEqual(ndmi_mean, None), -1, ndmi_mean
        )
        pixel_count = ee.Algorithms.If(
            ee.Algorithms.IsEqual(pixel_count, None), 0, pixel_count
        )
...
def compute_water_score(feature):
    ...
    best_info = ee.Algorithms.If(
        krz_value.gt(0), krz_max, ee.Algorithms.If(kr_value.gt(0), kr_max, k_max)
    )
...
def get_farthest_point(feature):
    ...
    def reducer(el, prev):
        ...
        return ee.Algorithms.If(
            el.getNumber("distance").gt(prev.getNumber("distance")), el, prev
        )

    max_coord = ee.Dictionary(
        coord_with_distances.iterate(
            reducer, ee.Dictionary({"coord": [0, 0], "distance": 0})
        )
    )

    farthest_point = ee.Geometry.Point(ee.List(max_coord.get("coord")))
    return (
        ee.Feature(farthest_point)
        .copyProperties(feature)
        .set("farthest_distance", max_coord.get("distance"))
    )
...
def generate_zoi_asset_on_gee(swb_asset_id, proj_id):
    ...
    elevation, cropping_mask, ndmi_image = calculate_elevation(
        landsat_collection, lulc_asset_id
    )
    ...
    computed_collection = swb_centroids.map(
        wrap_compute_metrics(elevation, ndmi_image, cropping_mask)
    )
    ...
    impactful, non_impactful, zoi_res = classify_checkdams(
        ndmis_all, poly_degree=8, threshold=0.010
    )
    features = []
    ...
    sync_project_fc_to_geoserver(
        zoi_with_water_props, proj_obj.name, layer_name_swb, "waterrej"
    )
    wait_for_task_completion(task)
