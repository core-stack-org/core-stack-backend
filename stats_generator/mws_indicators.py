    ...
    for specific_mws_id in df_hydrological_annual['UID'].unique():
        ...
        try:
            ...
            crp_Intensity_columns = df_crp_intensity_mws_data.filter(
                like='cropping_intensity_unit_less')  # cropping_intensity_avg
            ...
            cropping_intensity_avg = round(
                sum_crp_Intensity / total_crp_Intensity_column if total_crp_Intensity_column > 0 else 0, 4)

            ######### Cropping Intensity Trend  #################
            crp_intensity_T = df_crp_intensity_mws_data.filter(
                like='cropping_intensity_unit_less').dropna()  # Drop rows with NaN for trend calculation
            ...
            avg_single_cropped = round(
                percent_single_crop / total_single_crop_column if total_single_crop_column > 0 else 0, 4)
            ...
            avg_double_cropped = round(
                percent_double_crop / total_double_crop_column if total_double_crop_column > 0 else 0, 4)
            ...
            avg_triply_cropped = round(
                percent_triply_crop / total_triply_crop_column if total_triply_crop_column > 0 else 0, 4)
        ...
        if not df_swb_annual_mws_data.empty:
            ...
            if total_swb_area != 0:  # Check if total_swb_area is not zero
                ...
                avg_kharif_surface_water_mws = round(
                    sum_swb_area_kharif * 100 / total_swb_area_kharif_column if total_swb_area_kharif_column > 0 else 0,
                    4)
                ...
                avg_rabi_surface_water_mws = round(
                    sum_swb_rabi_area * 100 / total_swb_rabi_area_column if total_swb_rabi_area_column > 0 else 0, 4)
                ...
                avg_zaid_surface_water_mws = round(
                    sum_swb_zaid_area * 100 / total_swb_zaid_area_column if total_swb_zaid_area_column > 0 else 0, 4)
            else:
        ...
        ######### G Trend  #################
        G_Trend = hydro_annual_mws_data.filter(like='G').drop(
            columns=hydro_annual_mws_data.filter(like='DeltaG').columns).dropna()
        ...
        try:
            ...
            sum_moderate_severe = {year: 1 if (df_crpDrought_mws_data.iloc[0][f'Moderate_in_weeks_{year}'] +
                                               df_crpDrought_mws_data.iloc[0][f'Severe_in_weeks_{year}']) > 5 else 0 for
                                   year in years}
        ...
        try:
            ...
            degr_sum = \
            df_change_degr_detection_mws_data[['farm_to_barren_area_in_ha', 'farm_to_scrub_land_area_in_ha']].sum(
                axis=1).iloc[0]
            ...
            crp_sum = df_change_crp_detection_mws_data[
                ['double_to_single_area_in_ha', 'triple_to_double_area_in_ha', 'triple_to_single_area_in_ha']].sum(
                axis=1).iloc[0]
        ...
        try:
            df_change_affo_detection_mws_data = sheets['change_detection_afforestation'][
            ...
            afforestation_land_area = \
            df_change_affo_detection_mws_data.get('total_afforestation_area_in_ha', None).iloc[0]
        except:
            afforestation_land_area = 0

        ############  Change Detection Deforestation  ###################
        try:
            df_change_defo_detection_mws_data = sheets['change_detection_deforestation'][
                sheets['change_detection_deforestation']['UID'] == specific_mws_id]
            deforestation_column = ['forest_to_scrub_land_area_in_ha', 'forest_to_barren_area_in_ha',
                                    'forest_to_built_up_area_in_ha', 'forest_to_farm_area_in_ha']
            deforestation_land_area = \
            df_change_defo_detection_mws_data.get('total_deforestation_area_in_ha', None).iloc[0]
        except:
        ...
        try:
            ...
            urbanization_column = ['barren_shrub_to_built_up_area_in_ha', 'built_up_to_built_up_area_in_ha',
                                   'tree_farm_to_built_up_area_in_ha', 'water_to_built_up_area_in_ha']
            urbanization_land_area = df_change_urba_detection_mws_data.get('total_urbanization_area_in_ha', None).iloc[
                0]
        except:
        ...
        try:
            ...
            lulc_slope_category = df_lulc_slope_mws_data.get('cluster_name', pd.NA).iloc[
                0] if not df_lulc_slope_mws_data.empty else None
        except:
            lulc_slope_category = ''
        ...
        try:
            ...
            lulc_plain_category = df_lulc_plain_mws_data.get('cluster_name', pd.NA).iloc[
                0] if not df_lulc_plain_mws_data.empty else None
        except:
            lulc_plain_category = ''
        ...
        try:
            ...
            wide_scale_restoration = df_restoration_vector_mws_data.get('wide_scale_restoration_area_in_ha', None).iloc[
                0]
