            ...
            if response.content:
                ...
                if workspace == "terrain":
                    ...
                    create_excel_for_nrega_assets(
                        geojson_data, mws_geojson_datas, xlsx_file, writer
                    )
                    ...
                    create_excel_village_inters_mwss(
                        mws_geojson_datas, xlsx_file, writer, district, block
                    )
                    ...
                    create_excel_for_overall_tree_change(
...
def create_excel_for_ccd(data, xlsx_file, writer):
    ...
    for feature in features:
        ...
        for year in range(2017, 2023):
            row["High_Density_" + str(year)] = properties.get(
                "hi_de_" + str(year), None
            )
            row["Low_Density_" + str(year)] = properties.get("lo_de_" + str(year), None)
            row["Missing_Data_" + str(year)] = properties.get(
...
def create_excel_for_ch(data, xlsx_file, writer):
    ...
    for feature in features:
        ...
        for year in range(2017, 2023):
            ...
            row["Medium_Trees_" + str(year)] = properties.get(
...
def create_excel_chan_detection(xlsx_file, writer, district, block):
    ...
    merged_data = []
    all_uids = set(afforestation_data.keys()).union(
        set(cropintensity_data.keys()),
        set(deforestation_data.keys()),
        set(degradation_data.keys()),
        set(urbanization_data.keys()),
    )
    ...
    for feature in features:
        ...
        for num_uid_part in num_uid_parts_is:
            row = {
                ...
                "total_area_2017-2018": properties.get("area_17-18", 0)
                / 10000
                / len(num_uid_parts_is),
                "kharif_area_2017-2018": calculate_area(
                    area_17_18, properties.get("k_17-18", 0)
                )
                / len(num_uid_parts_is),
                "rabi_area_2017-2018": calculate_area(
                    area_17_18, properties.get("kr_17-18", 0)
                )
                / len(num_uid_parts_is),
                "zaid_area_2017-2018": calculate_area(
                    area_17_18, properties.get("krz_17-18", 0)
                )
                / len(num_uid_parts_is),
                "total_area_2018-2019": properties.get("area_18-19", 0)
                / 10000
                / len(num_uid_parts_is),
                "kharif_area_2018-2019": calculate_area(
                    properties.get("area_18-19", 0), properties.get("k_18-19", 0)
                )
                / len(num_uid_parts_is),
                "rabi_area_2018-2019": calculate_area(
                    properties.get("area_18-19", 0), properties.get("kr_18-19", 0)
                )
                / len(num_uid_parts_is),
                "zaid_area_2018-2019": calculate_area(
                    properties.get("area_18-19", 0), properties.get("krz_18-19", 0)
                )
                / len(num_uid_parts_is),
                "total_area_2019-2020": properties.get("area_19-20", 0)
                / 10000
                / len(num_uid_parts_is),
                "kharif_area_2019-2020": calculate_area(
                    properties.get("area_19-20", 0), properties.get("k_19-20", 0)
                )
                / len(num_uid_parts_is),
                "rabi_area_2019-2020": calculate_area(
                    properties.get("area_19-20", 0), properties.get("kr_19-20", 0)
                )
                / len(num_uid_parts_is),
                "zaid_area_2019-2020": calculate_area(
                    properties.get("area_19-20", 0), properties.get("krz_19-20", 0)
                )
                / len(num_uid_parts_is),
                "total_area_2020-2021": properties.get("area_20-21", 0)
                / 10000
                / len(num_uid_parts_is),
                "kharif_area_2020-2021": calculate_area(
                    properties.get("area_20-21", 0), properties.get("k_20-21", 0)
                )
                / len(num_uid_parts_is),
                "rabi_area_2020-2021": calculate_area(
                    properties.get("area_20-21", 0), properties.get("kr_20-21", 0)
                )
                / len(num_uid_parts_is),
                "zaid_area_2020-2021": calculate_area(
                    properties.get("area_20-21", 0), properties.get("krz_20-21", 0)
                )
                / len(num_uid_parts_is),
                "total_area_2021-2022": properties.get("area_21-22", 0)
                / 10000
                / len(num_uid_parts_is),
                "kharif_area_2021-2022": calculate_area(
                    properties.get("area_21-22", 0), properties.get("k_21-22", 0)
                )
                / len(num_uid_parts_is),
                "rabi_area_2021-2022": calculate_area(
                    properties.get("area_21-22", 0), properties.get("kr_21-22", 0)
                )
                / len(num_uid_parts_is),
                "zaid_area_2021-2022": calculate_area(
                    properties.get("area_21-22", 0), properties.get("krz_21-22", 0)
                )
                / len(num_uid_parts_is),
                ...
                "total_swb_area": properties.get("area_ored", 0)
                / 10000
                / len(num_uid_parts_is),
            }
            df_data.append(row)
...
def create_excel_village_nrega_assets(result_df, output_file, writer):
    ...
    for _, row in result_df.iterrows():
        ...
        if village_name not in counts:
            counts[village_name] = {
                "vill_id": village_id,
                **{
                    y: {cat: 0 for cat in workCategoryMapping.values()}
    ...
    for village_name, year_data in counts.items():
        row = {"vill_id": year_data["vill_id"], "vill_name": village_name}
...
def fetch_village_asset_count(state, district, block, writer, output_file):
    ...
    village_asset_count = (
        joined_gdf.groupby("vill_ID").size().reset_index(name="asset_count")
    )
...
def create_excel_crop_inten(data, output_file, writer):
    ...
    for feature in features:
        ...
        row = {
            ...
            "single_non_kharif_cropped_area_2017-2018": properties["single_n_1"]
            / 10000,
            ...
            "single_non_kharif_cropped_area_2018-2019": properties["single_n_2"]
            / 10000,
            ...
            "single_non_kharif_cropped_area_2019-2020": properties["single_n_3"]
            / 10000,
            ...
            "single_non_kharif_cropped_area_2020-2021": properties["single_n_4"]
            / 10000,
            ...
            "single_non_kharif_cropped_area_2021-2022": properties["single_n_5"]
            / 10000,
...
def get_generate_filter_mws_data(state, district, block):
    ...
    df_croppingDrought_kharif = pd.read_excel(
        xlsx_file, sheet_name="croppingDrought_kharif"
    )
    df_nrega_annual = pd.read_excel(xlsx_file, sheet_name="nrega_annual")
    df_mws_inters_villages = pd.read_excel(
        xlsx_file, sheet_name="mws_intersect_villages"
    )
    df_mws_change_detection = pd.read_excel(xlsx_file, sheet_name="change_detection")
    try:
        df_mws_terrain_lulc_slope = pd.read_excel(
            xlsx_file, sheet_name="terrain_lulc_slope"
        )
    except Exception as e:
        print(e)
    df_mws_terrain_lulc_plain = pd.read_excel(
        xlsx_file, sheet_name="terrain_lulc_plain"
    )

    results = []
    trend_info = {0: "no trend", 1: "increasing", -1: "decreasing"}
    ...
    for specific_mws_id in df_hydrological_annual["UID"].unique():
        ...
        try:
            terrain_vector_mws_data = df_terrain_vector[
                df_terrain_vector["UID"] == specific_mws_id
            ]
            terrainCluster_ID = terrain_vector_mws_data.get(
                "terrainCluster_ID", None
            ).iloc[0]  # terrain
        except:
            terrainCluster_ID = ""

        try:
            df_crp_intensity_mws_data = df_crp_intensity[
                df_crp_intensity["UID"] == specific_mws_id
            ]
            df_crp_intensity_mws_data = df_crp_intensity_mws_data.fillna(0)

            crp_Intensity_columns = df_crp_intensity_mws_data.filter(
                like="cropping_intensity"
        ...
        try:
            ...
            df_swb_annual_mws_data = df_swb_annual[
                df_swb_annual["UID"] == specific_mws_id
            ]
        ...
        #########  drought_category  ##############
        try:
            df_crpDrought_mws_data = df_croppingDrought_kharif[
                df_croppingDrought_kharif["UID"] == specific_mws_id
            ]
            years = ["2017", "2018", "2019", "2020", "2021", "2022"]
            sum_moderate_severe = {
                year: 1
                if (
                    df_crpDrought_mws_data.iloc[0][f"Moderate_{year}"]
                    + df_crpDrought_mws_data.iloc[0][f"Severe_{year}"]
                )
                > 5
                else 0
                for year in years
        ...
        ############## Nrega Asset ##########################
        try:
            df_nrega_assets_mws_data = df_nrega_annual[
                df_nrega_annual["mws_id"] == specific_mws_id
            ]
        ...
        try:
            df_mws_inters_villages_mws_data = df_mws_inters_villages[
                df_mws_inters_villages["MWS UID"] == specific_mws_id
            ]
        ...
        ############  Change Detection  ###################
        try:
            df_change_detection_mws_data = df_mws_change_detection[
                df_mws_change_detection["UID"] == specific_mws_id
            ]
            degradation_column = [
                "DEGR_Farm-Barren",
                "DEGR_Farm-Built_Up",
                "DEGR_Farm-Scrub_Land",
            ]
            afforestation_column = ["AFFO_Barren-Forest", "AFFO_Farm-Forest"]
            deforestation_column = [
                "DEFO_Forest-Scrub_land",
                "DEFO_Forest-Barren",
                "DEFO_Forest-Built_Up",
                "DEFO_Forest-Farm",
            ]
            urbanization_column = [
                "URBA_Barren/Shrub-Built_Up",
                "URBA_Built_Up-Built_Up",
                "URBA_Tree/Farm-Built_Up",
                "URBA_Water-Built_Up",
            ]

            degradation_land_area = df_change_detection_mws_data.get(
                "DEGR_Total", None
            ).iloc[0]
            afforestation_land_area = df_change_detection_mws_data.get(
                "AFFO_total", None
            ).iloc[0]
            deforestation_land_area = df_change_detection_mws_data.get(
                "DEFO_total", None
            ).iloc[0]
            urbanization_land_area = df_change_detection_mws_data.get(
                "URBA_Total", None
            ).iloc[0]
        except:
            degradation_land_area = afforestation_land_area = (
                deforestation_land_area
            ) = urbanization_land_area = 0

        ############# Terrain lulc slope / plain  #####################
        try:
            df_lulc_slope_mws_data = df_mws_terrain_lulc_slope[
                df_mws_terrain_lulc_slope["UID"] == specific_mws_id
            ]
            lulc_slope_category = (
        ...
        try:
            df_lulc_plain_mws_data = df_mws_terrain_lulc_plain[
                df_mws_terrain_lulc_plain["UID"] == specific_mws_id
            ]
            lulc_plain_category = (
...
def get_generate_filter_data_village(state, district, block):
    ...
    for specific_village_id in df_soc_eco_indi["village_id"].unique():
        ...
        total_assets = int(
            village_nrega_data.drop(columns=["vill_id", "vill_name"]).sum(axis=1).sum()
        )
