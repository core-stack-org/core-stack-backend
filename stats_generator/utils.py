...
def fetch_layers_for_excel_generation():
    ...
    """
    layers = LayerInfo.objects.filter(layer_type="vector", excel_to_be_generated=True).values("layer_name", "workspace",
                                                                                              "start_year", "end_year")
    return list(layers)
...
def create_excel_for_aquifer(data, xlsx_file, writer):
    ...
    principal_aquifers = [
        'Laterite', 'Basalt', 'Sandstone', 'Shale', 'Limestone',
        'Granite', 'Schist', 'Quartzite', 'Charnockite', 'Khondalite',
        'Banded Gneissic Complex', 'Gneiss', 'Intrusive', 'Alluvium', 'None'
    ]
    ...
    for uid, group in grouped:
        ...
        base_row = {
            'UID': uid,
            'area_in_ha': group['area_in_ha'].iloc[0],  # Should be same for all rows with same UID
    ...
    aquifer_cols = sorted([col for col in final_df.columns if col.startswith('principle_aq_')])
    ...
    for feature in features:
        ...
        for num_uid_part in num_uid_parts_is:
            ...
            for year in years:
                ...
                row[f'kharif_area_in_ha_{year}-{year + 1}'] = calculate_area(total_area, kharif_percentage) / len(
                    num_uid_parts_is)
                row[f'rabi_area_in_ha_{year}-{year + 1}'] = calculate_area(total_area, rabi_percentage) / len(
                    num_uid_parts_is)
                row[f'zaid_area_in_ha_{year}-{year + 1}'] = calculate_area(total_area, zaid_percentage) / len(
                    num_uid_parts_is)
...
def create_excel_for_nrega_assets(nrega_data, mws_data, output_file, writer, start_year, end_year):
    ...
    for _, row in joined.iterrows():
        ...
        if mws_id not in counts:
            counts[mws_id] = {year: {cat: 0 for cat in workCategoryMapping.values()} for year in
...
def fetch_village_asset_count(state, district, block, writer, output_file, start_year, end_year):
    ...
    try:
        ...
        create_excel_village_nrega_assets(result_df, output_file, writer, village_gdf[['vill_ID', 'vill_name']],
                                          start_year, end_year)
    ...
    result_df = pd.concat(
        [result_df, no_asset_villages[['vill_ID', 'vill_name', 'Asset ID', 'creation_t', 'WorkCatego']]],
        ignore_index=True)

    print("Processing complete")

    # 8. Create Excel file
    create_excel_village_nrega_assets(result_df, output_file, writer, village_gdf[['vill_ID', 'vill_name']], start_year,
                                      end_year)
