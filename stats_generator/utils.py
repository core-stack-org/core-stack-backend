...
def get_vector_layer_geoserver(state, district, block):
    ...
    base_path = os.path.join(path_to_add, "data/stats_excel_files")
...
def create_excel_village_nrega_assets(result_df, output_file, writer, start_year, end_year):
    workCategoryMapping = {
        "SWC - Landscape level impact": "Soil and water conservation",
        "Agri Impact - HH,  Community": "Land restoration",
        "Plantation": "Plantations",
    ...
    for _, row in result_df.iterrows():
        ...
        year = date_obj.year
        if year not in range(start_year, end_year):
            continue
        ...
        if village_name not in counts:
            counts[village_name] = {
                'vill_id': village_id,
                **{y: {cat: 0 for cat in workCategoryMapping.values()} for y in range(start_year, end_year)}
            }
...
def fetch_village_asset_count(state, district, block, writer, output_file, start_year, end_year):
    ...
    create_excel_village_nrega_assets(result_df, output_file, writer, start_year, end_year)
