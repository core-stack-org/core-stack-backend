...
def get_generate_filter_data_village(state, district, block):
    ...
    file_xl_path = os.path.join(EXCEL_PATH, 'data/stats_excel_files', state_folder, district_folder,
                                f"{district}_{block}")
    ...
    if not df_soc_eco_indi.empty and not df_nrega_village.empty:
        for specific_village_id in df_soc_eco_indi['village_id'].unique():
            ...
            total_assets = int(
                village_nrega_data.drop(columns=['vill_id', 'vill_name'], errors='ignore').sum(axis=1).sum())
