...
def get_generate_filter_data_village(state, district, block):
    ...
    if not os.path.exists(json_path):
        ...
        try:
            df_nrega_village = pd.read_excel(
                xlsx_file, sheet_name="nrega_assets_village"
            )
        except Exception as e:
        ...
        if not df_soc_eco_indi.empty:
            for specific_village_id in df_soc_eco_indi["village_id"].unique():
                ...
                total_population = village_id_data.get(
                    "total_population_count", None
                ).iloc[0]
                SC_percentage = round(
                    village_id_data.get("SC_percent", None).iloc[0], 4
                )
                ST_percentage = round(
                    village_id_data.get("ST_percent", None).iloc[0], 4
                )
                literacy_rate = round(
                    ...
                    feature["properties"].update(
                        {
                            ...
                            "percent_st_population": village_data[
                                "percent_st_population"
                            ],
                            "percent_sc_population": village_data[
                                "percent_sc_population"
                            ],
