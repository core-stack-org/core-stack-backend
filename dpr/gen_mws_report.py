    ...
    try:
        ...
        if block_detail:
            ...
            try:
                ...
                logger.error(
                    f"HTTP {e.response.status_code} error fetching Overpass data: {e.response.text[:200]}"
                )
                raise
        ...
        if final_data["lakes"] or final_data["reservoirs"]:
            ...
            if large_lakes or large_reservoirs:
                ...
                combined_water_bodies = sorted(
                    combined_water_bodies, key=lambda x: x["area_sq_m"], reverse=True
                )[:5]

                rname = [temp["name"] for temp in combined_water_bodies]
                rarea = [
                    str(round(temp["area_sq_m"] / 10000, 1))
                    for temp in combined_water_bodies
                ]
        ...
        if final_data["forests_mws"]:
            large_forests_mws = [
                f
                for f in final_data["forests_mws"]
                if f["area_sq_m"] >= MIN_AREA_THRESHOLD
            ]
        ...
        if final_data["lakes_mws"] or final_data["reservoirs_mws"]:
            large_lakes_mws = [
                lake
                for lake in final_data["lakes_mws"]
                if lake["area_sq_m"] >= MIN_AREA_THRESHOLD
            ]
            large_reservoirs_mws = [
                res
                for res in final_data["reservoirs_mws"]
                if res["area_sq_m"] >= MIN_AREA_THRESHOLD
            ]
            ...
            if large_lakes_mws or large_reservoirs_mws:
                ...
                combined_water_bodies_mws = sorted(
                    combined_water_bodies_mws,
                    key=lambda x: x["area_sq_m"],
                    reverse=True,
                )[:5]
                ...
                rarea = [
                    str(round(temp["area_sq_m"] / 10000, 1))
                    for temp in combined_water_bodies_mws
                ]
...
def get_terrain_data(state, district, block, uid):
    try:
        ...
        df["area_in_ha"] = pd.to_numeric(df["area_in_ha"], errors="coerce")
        df["hill_slope_area_percent"] = pd.to_numeric(
            df["hill_slope_area_percent"], errors="coerce"
        )
        df["plain_area_percent"] = pd.to_numeric(
            df["plain_area_percent"], errors="coerce"
        )
        df["ridge_area_percent"] = pd.to_numeric(
            df["ridge_area_percent"], errors="coerce"
        )
        df["slopy_area_percent"] = pd.to_numeric(
            df["slopy_area_percent"], errors="coerce"
        )
        df["valley_area_percent"] = pd.to_numeric(
            df["valley_area_percent"], errors="coerce"
        )

        area, hill_slope, plain_area, ridge_area, slopy_area, valley_area = df.loc[
            df["UID"] == uid,
            [
                "area_in_ha",
        ...
        selected_columns_cluster = [
            col for col in df.columns if col.startswith("terrain_description")
        ]
        ...
        if "terrain_lulc_slope" in excel_file.sheet_names:
            ...
            block_shrub_area = sum(
                df_slopes["shrub_scrubs_area_percent"] * df_slopes["area_in_ha"] / 100
            )
            block_barren_area = sum(
                df_slopes["barren_area_percent"] * df_slopes["area_in_ha"] / 100
            )
            block_tree_area = sum(
                df_slopes["forests_area_percent"] * df_slopes["area_in_ha"] / 100
            )
            block_kh_area = sum(
                df_slopes["single_kharif_area_percent"] * df_slopes["area_in_ha"] / 100
            )
            block_non_kh_area = sum(
                df_slopes["single_non_kharif_area_percent"]
                * df_slopes["area_in_ha"]
                / 100
            )
            block_double_area = sum(
                df_slopes["double_cropping_area_percent"]
                * df_slopes["area_in_ha"]
                / 100
            )
            block_triple_area = sum(
                df_slopes["triple_cropping_area_percent"]
                * df_slopes["area_in_ha"]
                / 100
            )
            ...
            if uid in df_slopes["UID"].values:
                (
                    ...
                    [
                        "area_in_ha",
                        "forests_area_percent",
                        "shrub_scrubs_area_percent",
                        "barren_area_percent",
                ...
                farmland_area = (
                    single_area_kh + single_area_non_kh + double_area + triple_area
                )
        ...
        if "terrain_lulc_plain" in excel_file.sheet_names:
            ...
            block_shrub_area = sum(
                df_plain["shrub_scrubs_area_percent"] * df_plain["area_in_ha"] / 100
            )
            block_barren_area = sum(
                df_plain["barren_area_percent"] * df_plain["area_in_ha"] / 100
            )
            block_tree_area = sum(
                df_plain["forests_area_percent"] * df_plain["area_in_ha"] / 100
            )
            block_single_area = sum(
                df_plain["single_kharif_area_percent"] * df_plain["area_in_ha"] / 100
            )
            block_double_area = sum(
                df_plain["double_cropping_area_percent"] * df_plain["area_in_ha"] / 100
            )
            block_triple_area = sum(
                df_plain["triple_cropping_area_percent"] * df_plain["area_in_ha"] / 100
            )
            ...
            block_lulc_area_plain[3] += (
                (block_single_area + block_double_area + block_triple_area)
                / total_block_area
            ) * 100
        ...
        if filtered_df[0] == "Broad Sloppy and Hilly":
            ...
            terrain_category_pct = (
                f"Slope: {round(percent_slope, 2)}% | Hill: {round(percent_hill, 2)}%"
            )
            ...
            terrain_category_pct = (
                f"Hill: {round(percent_hill, 2)}% | Valley: {round(percent_valley, 2)}%"
            )
        ...
        return (
            ...
            parameter_lulc,
            mws_lulc_area_slope,
            block_lulc_area_slope,
            mws_lulc_area_plain,
            block_lulc_area_plain,
            terrain_category_pct,
            round(percent_plain, 2),
            round(percent_slope, 2),
        )
...
def get_mws_barren_percent(state, district, block, uid):
    ...
    try:
        ...
        return round(
            (percent_slope / 100 * slope_barren_percent)
            + (percent_plain / 100 * plain_barren_percent),
            2,
        )
    except Exception as e:
...
def get_urbanization_sankey_data(state, district, block, uid):
    ...
    try:
        ...
        logger.info(
            f"Failed to read change_detection_urbanization sheet for {uid}: {e}"
        )
...
def get_change_detection_data(state, district, block, uid):
    try:
        ...
        # ? Land Degradation
        df_degrad["total_degradation_area_in_ha"] = df_degrad[
            "total_degradation_area_in_ha"
        ].apply(pd.to_numeric, errors="coerce")
        filtered_df = df_degrad.loc[
            df_degrad["UID"] == uid, "total_degradation_area_in_ha"
        ]
        ...
        df_defo["total_deforestation_area_in_ha"] = df_defo[
            "total_deforestation_area_in_ha"
        ].apply(pd.to_numeric, errors="coerce")
        filtered_df = df_defo.loc[
            df_defo["UID"] == uid, "total_deforestation_area_in_ha"
        ]
        ...
        df_urban["total_urbanization_area_in_ha"] = df_urban[
            "total_urbanization_area_in_ha"
        ].apply(pd.to_numeric, errors="coerce")
        filtered_df = df_urban.loc[
            df_urban["UID"] == uid, "total_urbanization_area_in_ha"
        ]
        ...
        df_restore["wide_scale_restoration_area_in_ha"] = df_restore[
            "wide_scale_restoration_area_in_ha"
        ].apply(pd.to_numeric, errors="coerce")
        filtered_df = df_restore.loc[
            df_restore["UID"] == uid, "wide_scale_restoration_area_in_ha"
        ]
        ...
        crop_intensity_sankey = get_crop_intensity_sankey_data(
...
def get_land_conflict_industrial_data(state, district, block, uid):
    try:
        ...
        conflicts = [
            {"title": title, "link": link} for title, link in zip(titles, links)
        ]
...
def get_factory_data(state, district, block, uid):
    try:
        ...
        def clean_address(address):
            ...
            address = re.sub(
                r"\s*(?:Fax|Email|Internet)\s*:.*$", "", address, flags=re.IGNORECASE
            )
...
def get_cropping_intensity(state, district, block, uid):
    try:
        ...
        if not filtered_df_inten.empty:
            ...
            avg_inten = sum(filtered_df_inten.values[0]) / len(
                filtered_df_inten.values[0]
            )
            num_years = (
                len(current_years)
                if current_years
                else len(filtered_df_inten.values[0])
            )
            ...
            for year in drought_years:
                selected_columns_d = [
                    ...
                    if col.startswith("cropping_intensity_unit_less_" + year)
                ]
            ...
            selected_columns_double = [
                col for col in df.columns if col.startswith("doubly_cropped_area_")
            ]
            selected_columns_triple = [
                col for col in df.columns if col.startswith("triply_cropped_area_")
            ]
            ...
            df[selected_columns_sum] = df[selected_columns_sum].apply(
                pd.to_numeric, errors="coerce"
            )
            ...
            if (
                not filtered_d_single.empty
                and not filtered_d_double.empty
                and not filtered_d_triple.empty
            ):

                for single, double, triple in zip(
                    filtered_d_single.values[0],
                    filtered_d_double.values[0],
                    filtered_d_triple.values[0],
                ):
        ...
        if res.status_code == 200:
            ...
            if features:
                year_cols = sorted(
                    [
                        k
                        for k in features[0]["properties"].keys()
                        if re.match(r"^area_\d{2}-\d{2}$", k)
                    ]
                )
                declining_count = 0
...
def get_surface_Water_bodies_data(state, district, block, uid):
    try:
        ...
        if not has_waterbody or filtered_df.empty:
            parameter_swb_1 = f"No surface water bodies were detected through remote sensing in this micro-watershed."
            return (
                "",
                parameter_swb_1,
                "",
        ...
        if not filtered_df.empty:
            ...
            total_wb_area = (
                float(df.loc[df["UID"] == uid, latest_total_col].values[0])
                if latest_total_col
                else 0
            )
            wb_area_percent = (
                round((total_wb_area / mws_area) * 100, 2) if mws_area else 0
            )

            wb_total_count, wb_large_count, wb_declining_percent = get_waterbody_stats(
                state, district, block, uid
            )
            ...
            try:
                ...
                drainage_density = (
                    round(
                        float(row_dd["drainage_density_std_in_km_per_km2"].values[0]), 2
                    )
                    if not row_dd.empty
                    else "-"
                )
            ...
            if river_count > 0:
                ...
                parameter_intro += (
                    f" and {canal_count} canal{'s' if canal_count != 1 else ''} are present."
                    if canal_count > 0
                    else "."
                )
            elif canal_count > 0:
                parameter_intro += (
                    f"{canal_count} canal{'s' if canal_count != 1 else ''} are present."
                )

            selected_columns_kh = [
                col for col in df.columns if col.startswith("kharif_area_in_ha_")
            ]
            ...
            df[selected_columns_kh] = df[selected_columns_kh].apply(
                pd.to_numeric, errors="coerce"
            )
            df_drought[selected_columns_moderate] = df_drought[
                selected_columns_moderate
            ...
            if len(drought_years):
                ...
                for year in drought_years:
                    selected_column_temp = [
                        col
                        for col in df.columns
                        if col.startswith("kharif_area_in_ha_" + year)
                    ]
                    if selected_column_temp:
                        yearly_area = df.loc[
                            df["UID"] == uid, selected_column_temp
                        ].values
                ...
                avg_area_d = (
                    total_area_d / len(drought_years) if len(drought_years) else 0
                )
                avg_area_nd = (
                    total_area_nd / len(non_drought_year)
            ...
            if len(non_drought_year):
                ...
                for year in non_drought_year:
                    ...
                    selected_column_temp_rb = [
                        col
                        for col in df.columns
                        if col.startswith("rabi_area_in_ha_" + year)
                    ]

                    if selected_column_temp:
                        yearly_area_kh = df.loc[
                            df["UID"] == uid, selected_column_temp
                        ].values
                    ...
                    if selected_column_temp_rb:
                        yearly_area_rb = df.loc[
                            df["UID"] == uid, selected_column_temp_rb
                        ].values
                ...
                if area_under_kh_nd > 0:
                    percent_rb_kh = (
                        (area_under_kh_nd - area_under_rb_nd) / area_under_kh_nd
                    ) * 100
            ...
            if len(drought_years):
                ...
                if area_under_kh > 0:
                    percent_rb_kh = (
                        (area_under_kh - area_under_rb) / area_under_kh
                    ) * 100
                    ...
                    if result.trend == "increasing":
                        ...
                        if (
                            hectare_drop_non_drought is None
                            or hectare_drop_drought > hectare_drop_non_drought
                        ):
                        ...
                        if (
                            percent_rb_kh_non_drought is None
                            or percent_rb_kh > percent_rb_kh_non_drought
                        ):
            ...
            selected_columns_rabi = [
                col for col in df.columns if col.startswith("rabi_area_in_ha_")
            ]
            selected_columns_zaid = [
                col for col in df.columns if col.startswith("zaid_area_in_ha_")
            ]
            ...
            if mws_area and filtered_df_rabi and filtered_df_zaid:
                avg_rabi_percent = round(
                    (sum(filtered_df_rabi) / len(filtered_df_rabi) / mws_area) * 100, 2
                )
                avg_zaid_percent = round(
                    (sum(filtered_df_zaid) / len(filtered_df_zaid) / mws_area) * 100, 2
                )
        ...
        print(e)
        logger.info(
            "Not able to access excel for %s state, %s district, %s block for Waterbodies",
            state.upper(),
            district.upper(),
            block.upper(),
        )
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            if features:
                ...
                date_keys = sorted(
                    [k for k in props.keys() if re.match(r"^\d{4}-\d{2}-\d{2}$", k)]
                )
                ...
                for date_key in date_keys:
                    try:
                        ...
                        precip_data.append(
                            round(float(val.get("Precipitation") or 0), 2)
                        )
...
def _fetch_ndvi_timeseries(state, district, block, uid, land_cover):
    ...
    try:
        ...
        logger.info(
            f"Failed to fetch NDVI ({land_cover}) timeseries data for {uid}: {e}"
        )
...
def get_water_balance_data(state, district, block, uid):
    try:
        ...
        df[selected_column_dg] = df[selected_column_dg].apply(
            pd.to_numeric, errors="coerce"
        )
        df[selected_column_g] = df[selected_column_g].apply(
            pd.to_numeric, errors="coerce"
        )
        ...
        if len(non_drought_years):
            ...
            for year in non_drought_years:
                ...
                # ? Rainfall
                selected_column_precp = [
                    col
                    for col in df.columns
                    if col.startswith("Precipitation_in_mm_" + year)
                ]
                if selected_column_precp:
                    rainfall_data = df.loc[
                        df["UID"] == uid, selected_column_precp
                    ].values
                ...
                # ? Monsoon Onset
                selected_column_onset = [
                    col
                    for col in df_drought.columns
                    if col.startswith("monsoon_onset_" + year)
                ]
                if selected_column_onset:
                    onset_data = df_drought.loc[
                        df_drought["UID"] == uid, selected_column_onset
                    ].values
                ...
                selected_column_kh = [
                    ...
                    if col.startswith("delta g_kharif_in_mm_" + year)
                ]
                selected_column_rb = [
                    col
                    for col in df_seasonal.columns
                    if col.startswith("delta g_rabi_in_mm_" + year)
                ]
                selected_column_zd = [
                    col
                    for col in df_seasonal.columns
                    if col.startswith("delta g_zaid_in_mm_" + year)
                ]
                ...
                if selected_column_kh:
                    delg_kh_data = df_seasonal.loc[
                        df_seasonal["UID"] == uid, selected_column_kh
                    ].values
                    if len(delg_kh_data) > 0 and len(delg_kh_data[0]) > 0:
                        delg_kh_val = delg_kh_data[0][0]

                if selected_column_rb:
                    delg_rb_data = df_seasonal.loc[
                        df_seasonal["UID"] == uid, selected_column_rb
                    ].values
                    if len(delg_rb_data) > 0 and len(delg_rb_data[0]) > 0:
                        delg_rb_val = delg_rb_data[0][0]

                if selected_column_zd:
                    delg_zd_data = df_seasonal.loc[
                        df_seasonal["UID"] == uid, selected_column_zd
                    ].values
                ...
                # ? Runoff
                selected_column_runoff = [
                    col for col in df.columns if col.startswith("RunOff_in_mm_" + year)
                ]
                if selected_column_runoff:
                    runoff_data = df.loc[
                        df["UID"] == uid, selected_column_runoff
                    ].values
            ...
            good_rainfall += original_string.replace(
            ...
            if min_date != None and max_date != None:
                good_rainfall += (
                    f" with monsoon onset between [{min_date}, {max_date}]."
                )
            else:
                good_rainfall += f"."
    ...
    try:
        ...
        try:
            ...
            if not row_dem.empty:
                ...
                mean_elev = (
                    round(float(row_dem["mean_elevation_in_m"].values[0]), 2)
                    if "mean_elevation_in_m" in row_dem.columns
                    else "-"
                )
                relief = (
                    round(max_elev - min_elev, 2)
        ...
        try:
            ...
            aquifer_class = (
                row_aq["aquifer_class"].values[0] if not row_aq.empty else "-"
            )
        except Exception as e:
        ...
        try:
            ...
            soge_dev_percent = (
                round(float(row_soge["soge_dev_percent"].values[0]), 2)
                if not row_soge.empty and "soge_dev_percent" in row_soge.columns
                else "-"
            )
        except Exception as e:
...
def get_terrain_and_lulc_data(state, district, block, uid):
    try:
        ...
        def get_vals(df, u):
            ...
            c = (
                val("single_kharif_area_percent")
                + val("single_non_kharif_area_percent")
                + val("double_cropping_area_percent")
                + val("triple_cropping_area_percent")
            )
...
def get_cropping_water_hydro_data(state, district, block, uid):
    try:
        ...
        try:
            ...
            single_kharif = get_latest_col_val(
                df_crop, "single_kharif_cropped_area_in_ha_", uid
            )
            ...
            single_non = get_latest_col_val(
                df_crop, "single_non_kharif_cropped_area_in_ha_", uid
            )
        ...
        try:
            ...
            water_kharif = round(
                get_latest_col_val(df_swb, "kharif_area_in_ha_", uid), 2
            )
            ...
            logger.info(
                f"Failed to read surfaceWaterBodies_annual sheet for {uid}: {e}"
            )
        ...
        try:
            ...
            rf_kharif = round(
                get_latest_col_val(df_hydro, "precipitation_kharif_in_mm_", uid), 2
            )
            rf_rabi = round(
                get_latest_col_val(df_hydro, "precipitation_rabi_in_mm_", uid), 2
            )
            rf_zaid = round(
                get_latest_col_val(df_hydro, "precipitation_zaid_in_mm_", uid), 2
            )
            ...
            ro_kharif = round(
                get_latest_col_val(df_hydro, "runoff_kharif_in_mm_", uid), 2
            )
...
def get_soge_data(state, district, block, uid):
    try:
        ...
        if aquifer_class == "Alluvium":
            ...
            selected_column_g = [
                col for col in df_hydro.columns if col.startswith("G_")
            ]
            df_hydro[selected_column_g] = df_hydro[selected_column_g].apply(
                pd.to_numeric, errors="coerce"
            )
            filtered_df_g = df_hydro.loc[
                df_hydro["UID"] == uid, selected_column_g
            ].values[0]
            ...
            if soge_class == "Safe":
                ...
                parameter_soge += (
                    f"Extraction exceeds recharge; groundwater levels falling sharply."
                )
...
def get_drought_data(state, district, block, uid):
    try:
        ...
        selected_columns_drysp_all = [
            col for col in df.columns if col.startswith("drysp_unit_4_weeks")
        ]
        df[selected_columns_drysp_all] = df[selected_columns_drysp_all].apply(
            pd.to_numeric, errors="coerce"
        )
        filtered_df_drysp_all = (
            df.loc[df["UID"] == uid, selected_columns_drysp_all].values[0].tolist()
        )
        ...
        if len(drought_years):
            ...
            selected_columns_drysp = [
                col
                for col in df.columns
                if any(
                    col.startswith(f"drysp_unit_4_weeks_{year}")
                    for year in drought_years
                )
            ]
            ...
            if len(drysp_tuple) > 0:
                ...
                for index, item in enumerate(drysp_tuple):
                    if index < len(drysp_tuple) - 1:
                        ...
                        formatted_sentence += (
                            f"and in {item[1]} lasted {item[0]} weeks."
                        )
            ...
            for id in villages:
                ...
                if has_nrega and df_village is not None:
                    ...
                    if len(village_name_col) > 0:
                        village_match = df_village.loc[
                            df_village["vill_id"] == id, village_name_col
                        ]
                ...
                if village_name is None and has_socio and df_socio is not None:
                    village_name_col = [
                        ...
                        if col.startswith("village_name")
                    ]
                    if len(village_name_col) > 0:
                        village_match = df_socio.loc[
                            df_socio["village_id"] == id, village_name_col
                        ]
                        if not village_match.empty:
                            name = village_match.values[0].tolist()
                            village_name = name[0] if name else None
                ...
                if has_nrega and df_village is not None:
                    ...
                    if len(swc_cols) > 0:
                        ...
                        village_match = df_village.loc[
                            df_village["vill_id"] == id, swc_cols
                        ]
                        if not village_match.empty:
                    ...
                    if len(plant_cols) > 0:
                        ...
                        village_match = df_village.loc[
                            df_village["vill_id"] == id, plant_cols
                        ]
                        if not village_match.empty:
                            plantation_work.append(
                                sum(village_match.values[0].tolist())
                            )
                        else:
                    ...
                    if len(iof_cols) > 0:
                        ...
                        village_match = df_village.loc[
                            df_village["vill_id"] == id, iof_cols
                        ]
                        if not village_match.empty:
                    ...
                    if len(ofl_cols) > 0:
                        ...
                        village_match = df_village.loc[
                            df_village["vill_id"] == id, ofl_cols
                        ]
                        if not village_match.empty:
                    ...
                    if len(ofw_cols) > 0:
                        ...
                        village_match = df_village.loc[
                            df_village["vill_id"] == id, ofw_cols
                        ]
                        if not village_match.empty:
                ...
                if has_socio and df_socio is not None:
                    ...
                    if len(sc_percent_col) > 0:
                        ...
                        village_match = df_socio.loc[
                            df_socio["village_id"] == id, sc_percent_col
                        ]
                        if not village_match.empty:
                    ...
                    if len(st_percent_col) > 0:
                        ...
                        village_match = df_socio.loc[
                            df_socio["village_id"] == id, st_percent_col
                        ]
                        if not village_match.empty:
                    ...
                    if len(pop_col) > 0:
                        ...
                        village_match = df_socio.loc[
                            df_socio["village_id"] == id, pop_col
                        ]
                        if not village_match.empty:
