...
def generate_dpr(request):
    try:
        ...
        logger.info("Generating DPR for plan ID: %s and email ID: %s", plan_id, email_id)
        ...
        return Response({"message": f"DPR generated successfully and sent to the email ID: {email_id}"}, status=status.HTTP_201_CREATED)
