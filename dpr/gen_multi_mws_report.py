...
def get_mws_data(state, district, block, mwsList, filters):
    try:
        df = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="mws_intersect_villages")
...
def get_terrain_mws_data(state, district, block, mwsList):
    try:
        df = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="terrain")
...
def get_lulc_mws_data(state, district, block, mwsList):
    try:
        excel_file = pd.ExcelFile(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx")
        ...
        if "terrain_lulc_slope" in excel_file.sheet_names:
            df_slope = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="terrain_lulc_slope")
            ...
            df_filtered_uid["area_of_forest"] = (df_filtered_uid["area_in_hac"] * df_filtered_uid["% of area forests"]) / 100
            df_filtered_uid["area_of_shrub_scrubs"] = (df_filtered_uid["area_in_hac"] * df_filtered_uid["% of area shrub_scrubs"]) / 100
            df_filtered_uid["area_of_barren"] = (df_filtered_uid["area_in_hac"] * df_filtered_uid["% of area barren"]) / 100
        ...
        if "terrain_lulc_plain" in excel_file.sheet_names:
            df_plain = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="terrain_lulc_plain")

            df_plain["% of area single_non_kharif"] = pd.to_numeric(df_plain["% of area single_non_kharif"], errors='coerce')
            df_plain["% of area single_cropping"] = pd.to_numeric(df_plain["% of area single_cropping"], errors='coerce')
            df_plain["% of area double cropping"] = pd.to_numeric(df_plain["% of area double cropping"], errors='coerce')
            df_plain["% of area triple cropping"] = pd.to_numeric(df_plain["% of area triple cropping"], errors='coerce')
            ...
            df_filtered_uid["area_of_single_non_kharif"] = (df_filtered_uid["area_in_hac"] * df_filtered_uid["% of area single_non_kharif"]) / 100
            df_filtered_uid["area_of_single_cropping"] = (df_filtered_uid["area_in_hac"] * df_filtered_uid["% of area single_cropping"]) / 100
            df_filtered_uid["area_of_double_cropping"] = (df_filtered_uid["area_in_hac"] * df_filtered_uid["% of area double cropping"]) / 100
            df_filtered_uid["area_of_triple_cropping"] = (df_filtered_uid["area_in_hac"] * df_filtered_uid["% of area triple cropping"]) / 100
...
def get_degrad_mws_data(state, district, block, mwsList):
    try:
        ...
        if "change_detection_degradation" in excel_file.sheet_names:
            ...
            df_degrad = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="change_detection_degradation")
...
def get_reduction_mws_data(state, district, block, mwsList):
    try:
        ...
        if "change_detection_deforestation" in excel_file.sheet_names:
            ...
            df_reduce = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="change_detection_deforestation")
...
def get_urban_mws_data(state, district, block, mwsList):
    try:
        ...
        if "change_detection_urbanization" in excel_file.sheet_names:
            ...
            df_urban = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="change_detection_urbanization")
...
def get_cropping_mws_data(state, district, block, mwsList):
    try:
        df_crop = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="croppingIntensity_annual")

        df_drought = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="croppingDrought_kharif")
        ...
        df_filtered_uid[selected_columns_single] = df_filtered_uid[selected_columns_single].apply(pd.to_numeric, errors="coerce")
        df_filtered_uid[selected_columns_double] = df_filtered_uid[selected_columns_double].apply(pd.to_numeric, errors="coerce")
        df_filtered_uid[selected_columns_triple] = df_filtered_uid[selected_columns_triple].apply(pd.to_numeric, errors="coerce")
        df_filtered_uid[selected_columns_sum] = df_filtered_uid[selected_columns_sum].apply(pd.to_numeric, errors="coerce")
        ...
        df_filtered_uid["avg_doubly_cropped_area_percent"] = np.where(df_filtered_uid["sum"] != 0,(df_filtered_uid["avg_doubly_cropped_area"] / df_filtered_uid["sum"]) * 100, 0)
...
def get_surface_wb_mws_data(state, district, block, mwsList):
    try:
        ...
        df_swb =  pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="surfaceWaterBodies_annual")
        
        df_terrain = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="terrain")
        ...
        df_filtered_uid_swb[selected_columns] = df_filtered_uid_swb[selected_columns].apply(pd.to_numeric, errors="coerce")
        ...
        df_filtered_uid_swb[selected_columns_rabi] = df_filtered_uid_swb[selected_columns_rabi].apply(pd.to_numeric, errors="coerce")
...
def get_water_balance_mws_data(state, district, block, mwsList):
    try:
        ...
        df_hydro = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="hydrological_annual")

        df_drought = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="croppingDrought_kharif")

        df_fortnight = pd.read_excel(BASE_DIR+state.upper()+"/"+district.upper()+"/"+district.lower()+"_"+block.lower()+".xlsx", sheet_name="hydrological_seasonal")
        ...
        for uid in mwsList:
            ...
            for year in top_years_nd:
                selected_columns_dg_kh = [col for col in df_fortnight.columns if col.startswith("delta g_kharif_" + year)]
                ...
                df_fortnight[selected_columns_dg_kh] = df_fortnight[selected_columns_dg_kh].apply(pd.to_numeric, errors="coerce")
                df_fortnight[selected_columns_dg_rb] = df_fortnight[selected_columns_dg_rb].apply(pd.to_numeric, errors="coerce")
                df_fortnight[selected_columns_dg_zd] = df_fortnight[selected_columns_dg_zd].apply(pd.to_numeric, errors="coerce")
