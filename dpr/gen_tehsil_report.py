    ...
    try:
        ...
        if final_data["lakes"] or final_data["reservoirs"]:
            ...
            if large_lakes or large_reservoirs:
                ...
                if len(rname) == 1:
                    parameter_block += f"waterbody {rname[0]} ({rarea[0]} area)"
                else:
                    for i in range(len(rname)):
                        if i == len(rname) - 1:
                            parameter_block += (
                                f"and waterbody {rname[i]} ({rarea[i]} area)"
                            )
                        else:
                            parameter_block += (
                                f"waterbody {rname[i]} ({rarea[i]} area), "
                            )
...
def get_pattern_intensity(state, district, block):
    try:
        ...
        return {
            ...
            "active_patterns": sorted(list(active_patterns)),  # Return as sorted list
        }
