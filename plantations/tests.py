...
class KMLFileModelTest(TestCase):
    ...
    def test_kml_file_creation(self):
        ...
        kml_file = KMLFile.objects.create(
            project=self.project, name="Test KML", file=temp_file, uploaded_by=self.user
        )

        # Check model attributes
        self.assertEqual(kml_file.name, "Test KML")
        self.assertEqual(kml_file.project, self.project)
...
class KMLFileAPITest(APITestCase):
    def setUp(self):
        ...
        self.kml_list_url = reverse(
            "project-kml-list", kwargs={"project_pk": self.project.pk}
        )
    ...
    def test_upload_kml_file(self):
        ...
        response = self.client.post(
            self.kml_list_url,
            {"name": "Test KML", "file": SimpleUploadedFile("test.kml", content)},
            format="multipart",
        )
    ...
    def test_duplicate_kml_detection(self):
        ...
        response1 = self.client.post(
            self.kml_list_url,
            {"name": "Test KML 1", "file": SimpleUploadedFile("test1.kml", content)},
            format="multipart",
        )
        ...
        response2 = self.client.post(
            self.kml_list_url,
            {"name": "Test KML 2", "file": SimpleUploadedFile("test2.kml", content)},
            format="multipart",
        )
