...
from utilities.constants import WHATSAPP_MEDIA_PATH
...
def convertTokenExpireTime(time_str):
    ...
    datetime_object = datetime.strptime(time_str_formated, "%Y-%m-%d %H:%M:%S.%f")
...
def generateWhatsppToken(token_url, username, password):
    ...
    response = requests.post(
        token_url.strip(),
        auth=HTTPBasicAuth(str(username.strip()), str(password.strip())),
    )
    print(response.text)
...
def create_user(app_type, app_instance_config, contact_number):
    user = Interface.models.Users(
        app_type=app_type,
        app_instance_config=app_instance_config,
        msisdn=contact_number,
        current_smj=app_instance_config.smj,
    )
    user.save()
    return user
...
def get_items_query(item_count, theme, ai):
    ...
    try:
        ...
        response = table.query(
            ...
            & Key("skey").eq(skey)  # & Key('channel').eq(channel)
        )
        item = response["Items"]
...
def create_item_url(user, items_list):
    ...
    for i, item in enumerate(items_list):
        print(item)
        result_item = (
            "http://api3.gramvaani.org/user/share/audio?i="
            + str(item)
            + ".mp3&d=1cbd1fae55a04320&u="
            + u_data
        )
...
def create_item_url_new(user, items_list_from_query, server, rl):
    ...
    for item_tuple in items_list_from_query:
        if len(item_tuple) == 5:
            ...
            item = str(item)
            result_item = (
                "https://appserver.gramvaani.org/share?t=item&i="
                + pri_key
                + "&rl="
                + rl
                + "&app="
                ...
                + "&u="
                + u_data
                + "&utm_source=gv_whatsapp&utm_medium=gv_whatsapp_bot&utm_campaign=gv_"
                + smj_name
            )  # eX0Mch4g"
            ...
            item = str(item)
            result_item = (
                "https://appserver.gramvaani.org/share?t=item&i=TPH4-ai-"
                + str(ai)
                + "-"
                + item
                + "&rl="
            ...
            if user.misc_data:
                ...
                if user_misc_data.get("server") and user_misc_data.get(theme):
                    ...
                    for data in data_dict.get("data"):
                        ...
                        if data.get("getContentBy") == "tags":
                            ...
                            server, static_publist = (
                                Interface.api.getDataByElasticSearch(query)
                            )
                            ...
                            print(
                                "static publist after using getDatabyElasticSearch ",
                                static_publist,
                            )
                            ...
                            publistch, query = get_items_query(
                                items_to_query, theme, ai
                            )  # end
                            print("PUBLISTCH >>>>>", publistch)
                            print("QUERY >>>>>> ", query)
                        print("query in fetch items: ", query)
                        server, static_publist = Interface.api.getDataByElasticSearch(
                            query
                        )
                        print(
                            "static publist after using getDatabyElasticSearch ",
                        ...
                        try:
                            ...
                            print(
                                "static_item_list in fetch items ::", static_item_list
                            )
                        except ClientError as e:
                            static_item_list = []
                            print(e.response["Error"]["Message"])
                    ...
                    print(
                        " when user.misc_data is present but server not in user.misc_data >>>>>"
                    )
                    for data in data_dict.get("data"):
                        source = data["source"]
                        item_count = data["count"]
                        if data.get("getContentBy") == "tags":
                            ...
                            try:
                                ...
                                static_item_list = data[0][publistch]
                                print(
                                    "static_item_list in fetch items ::",
                                    static_item_list,
                                )
                            except ClientError as e:
                                static_item_list = []
                ...
                for data in data_dict.get("data"):
                    ...
                    if data.get("getContentBy") == "tags":
                        param = data["getContentBy"]
                        print(
                            "Params to be passed in query :: PPPPPPTTTTT ", param, theme
                        )
                        query = get_items_query_by_params(param, theme)
                        print("query in fetch items: PPPP", query)
                        server, static_publist = Interface.api.getDataByElasticSearch(
...
def get_qna_model_result(
    in_ques,
    relevant_questions,
    sanitized_question_ids,
    sanitized_questions,
    comment_urls,
    ...
    if len(word_list1) <= 2:
        default_ids = list(
            OrderedDict.fromkeys(sanitized_question_ids)
        )  # Get only unique IDs
        default_ids = [int(i) for i in default_ids if i != ""]
        default_ids = default_ids[:3]  # take only top 3

        default_sanitized_questions = list(
            OrderedDict.fromkeys(sanitized_questions)
        )  # Get only unique IDs
        ...
        default_comment_urls = list(
            OrderedDict.fromkeys(comment_urls)
        )  # Get only unique IDs
        ...
        for i in range(len(default_sanitized_questions)):
            default_list.append({default_sanitized_questions[i]: comment_urls[i]})
        ...
        ques_response["error_msg"] = (
            "आपने अपना सवाल तीन शब्दों से कम में लिखा है। कृपया अपना सवाल और विस्तार से लिखें।"
        )
    ...
    if sorted_score[0][1] < similarity_threshold:
        ...
        ques_response["error_msg"] = (
            "माफ़ करिए, आपके पूछे गए सवाल का जवाब इस वक़्त हमारे पास नहीं है"
        )
        ...
        for idx in range(len(sorted_score)):
            if (
                ...
                not in visited_sanitized_questions
            ):
...
def google_speech_recognition(media_path):
    ...
    print("File =", file)
    subprocess.call(["ffmpeg", "-i", media_path, file])
...
def flat_df(df):
    final_df = df.drop_duplicates(
        subset=["Sanitized Question", "Sanitized question ID", "Comment_URL"],
        keep=False,
    )
    rows = []
    for row in final_df.itertuples():
        row_list = [
            row["Sanitized Question"],
            row["Sanitized question ID"],
            row["Comment_URL"],
        ]
