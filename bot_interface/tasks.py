...
def StartUserSession(
    ...
    try:
        ...
        if a:
            ...
            if is_in_community:
                logger.info(
                    "User %s is in community, proceeding with session", user_number
                )
                ...
                current_state = (
                    event_packet.get("state") or bot_instance.init_state
                )  # Preserve current state
                current_smj_id = (
                    event_packet.get("smj_id") or bot_instance.smj.id
                )  # Preserve SMJ context

                print(
                    f"Community user button event - Event: {current_event}, State: {current_state}, SMJ: {current_smj_id}"
                )
                ...
                logger.info(
                    "User %s is not in community, handling onboarding", user_number
                )
                ...
                print(
                    f"User check result - ID: {bot_user_id}, is_new_user: {is_new_user}"
                )

                # Update event packet with user information
                event_packet.update(
                    {"user_id": bot_user_id, "is_new_user": is_new_user}
                )
                ...
                current_session = (
                    []
                )  # Initialize current_session to avoid UnboundLocalError
                if is_new_user:
                    ...
                    try:
                        ...
                        logger.error(
                            "Error creating UserSession for new user: %s", str(e)
                        )
                        return
                else:
                    ...
                    try:
                        ...
                        if len(user_session.current_session) == 0:
                            print(
                                "UserSession current_session is empty, updating event_packet"
                            )
                            ...
                            if event_packet.get("type") in [
                                ...
                                print(
                                    f"Preserving existing interaction event: {event_to_set}"
                                )
                            else:
                                ...
                                event_type = True
                                print(
                                    "Setting event to onboarding for new conversation"
                                )
                            ...
                            updated_event = (
                                event if event_packet.get("event") else "success"
                            )
                            ...
                            print("=" * 50)
                            print(
                                "Current session in existing session:", current_session
                            )
                            print("=" * 50)
                            ...
                            if event_type == "location":
                                ...
                                location_data = {
                                    ...
                                    "latitude": event_packet.get("misc", {}).get(
                                        "latitude", ""
                                    ),
                                    "longitude": event_packet.get("misc", {}).get(
                                        "longitude", ""
                                    ),
                                    "address": event_packet.get("misc", {}).get(
                                        "address", ""
                                    ),
                                    "name": event_packet.get("misc", {}).get(
                                        "name", ""
                                    ),
                                }
                                ...
                                print(
                                    f"Appended audio data to session: {audio_key} = {audio_data}"
                                )
                        ...
                        logger.info(
                            "No existing UserSession found for user, creating new one"
                        )
                        try:
                            user_session = (
                                bot_interface.models.UserSessions.objects.create(
                                    bot=bot_instance,
                                    user_id=bot_user_id,
                                    phone=event_packet.get("wa_id", user_number),
                                    app_type=app_type,
                            ...
                            print(
                                f"Created new UserSession for existing user: {user_session}"
                            )
                            event_packet.update(
                                {
                ...
                print(
                    f"Session flags - start_session: {start_session}, event_type: {event_type}"
                )
                logger.info(
                    "Current event packet after checking start session and event type flags: %s",
                    event_packet,
                )  # Debugging current session content
                print("=" * 50)
                print(
                    "Current session content:", user_session.current_session
                )  # Debugging current session content
                print("=" * 50)
                ...
                try:
                    smj = bot_interface.models.SMJ.objects.get(
                        id=smj_id
                    )  # Use .get() to get single object
                    ...
                    logger.error(
                        "Error parsing SMJ JSON tasks.py line 148: %s", json_err
                    )
                    return
                ...
                try:
                    ...
                    print(
                        f"Updated user session context: SMJ ID {smj_id}, State: {event_state}"
                    )
                except bot_interface.models.SMJ.DoesNotExist:
                    print(
                        f"Warning: SMJ with ID {smj_id} not found for context preservation"
                    )
