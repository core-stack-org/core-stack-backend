...
The multi-tenant system centers around three core abstractions: **Organizations** as top-level tenants, **Users** as system actors, and **Projects** as organizational workspaces with scoped permissions. The architecture follows a hierarchical isolation model where superadmins maintain global control, organization admins manage tenant-level resources, and project-specific roles control access to individual workspaces.
...
The system integrates with Open Data Kit (ODK) for field data collection through organization-level ODK project mapping. Each organization can be associated with a specific ODK project, and users within the organization can have individual ODK credentials for data synchronization.
...
| `odk_username` | User | User's ODK authentication identifier |
...
Sources: [nrm\_app/settings.py](../nrm_app/settings.py#L271), [nrm\_app/settings.py](../nrm_app/settings.py#L60)
